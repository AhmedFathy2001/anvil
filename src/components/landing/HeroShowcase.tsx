...
export default function HeroShowcase() {
  ...
  return (
    <div className="rounded-2xl border border-card-border bg-brown-dark/70 p-3.5 shadow-[0_24px_70px_-35px_rgba(0,0,0,0.95)] sm:p-4">
      ...
      {/*
        ALL FIVE PANELS SHARE ONE CELL, and the inactive ones are hidden rather than unmounted.
        Rendering only the active panel made the box as tall as whatever you had clicked — a 25-tile
        board, then four rows of text — so the tabs slid out from under the cursor and the headline
        beside them jumped on every click. The panels are different heights and always will be; the
        fix is to reserve the tallest, and a grid whose children all sit at row 1 / column 1 does
        that fluidly at any width, with nothing measured and no magic number to go stale.

        `invisible`, not `hidden`: visibility keeps the box in flow, which is the whole point.
      */}
      <div className="grid [&>*]:col-start-1 [&>*]:row-start-1">
        {FORMATS.map((f) => (
          <div
            key={f.key}
            aria-hidden={format !== f.key}
            className={format === f.key ? '' : 'invisible'}
          >
            {f.key === 'bingo' && (
              <>
                <Head title="Summer Bingo — Team Molten" meta={`${DONE.length} / 25`} />
                {/* boardSize is N, the grid's EDGE — not the tile count. Passing 25 laid the board
                    out as one 25-wide strip with its own scrollbar. */}
