...
import { db } from '@/db';
import { clanMembers, eventSignups, events, players, signupFees, teams } from '@/db/schema';
...
export async function placeCaptainOnTeam(
  ...
  const existing = await db
    .select({ id: players.id, teamId: players.teamId, clanMemberId: players.clanMemberId })
    .from(players)
