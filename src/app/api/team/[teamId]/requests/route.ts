...
import { db } from '@/db';
import { clanMembers, eventSignups, players, users } from '@/db/schema';
...
export async function POST(
  ...
  const existingPlayer = await db.query.players.findFirst({
    where: and(eq(players.eventId, signup.eventId), eq(players.clanMemberId, signup.clanMemberId)),
  });
  if (!existingPlayer) {
    ...
    await db.update(players).set({ teamId: tId }).where(eq(players.id, existingPlayer.id));
  }
