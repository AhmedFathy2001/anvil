...
/**
 * A way to actually reach this person.
 *
 * `effectiveDiscordId` has been on every roster row since the column was added and was rendered
 * nowhere — so the one screen listing everybody in the clan could tell you their rank, their status
 * and when they last played, and gave you no way to say anything to them. The id is the durable
 * handle (a username can change; a snowflake cannot), and discord.com/users/<id> opens a profile you
 * can message.
 */
function DiscordLink({ id, rsn }: { id: string | null | undefined; rsn: string }) {
  if (!id) return null;
  return (
    <a
                ...
                >
                  ...
                  <td className="px-4 py-3 font-medium">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span>{m.rsn}</span>
                      <DiscordLink id={m.effectiveDiscordId ?? m.discordId} rsn={m.rsn} />
                      <AccountBadge m={m} />
                    </div>
                    {m.pendingRole && (
                      <div className="mt-1">
